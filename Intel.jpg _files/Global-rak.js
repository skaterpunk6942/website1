...
function prBasketCallback(data) {
	var obj = eval('(' + data + ')');
	var qsParam = "";
	var basketLink = "http://cart.rakuten.com/CO/ConfirmBasket.aspx";

	if (location.host.indexOf("beta.rakuten.com") > -1)
		basketLink = "http://cart.beta.rakuten.com/CO/ConfirmBasket.aspx";
	else if (location.host.indexOf("test.rakuten.com") > -1)
		basketLink = "http://test.cart.rakuten.com/CO/ConfirmBasket.aspx";
...
function prShowAddToCartSpinner() {
	...
	$('#addItemMsg').html('<img src="http://images.rakuten.com/buy_assets/search/ajax-loader.gif" style="margin-left: 176px; margin-top: 27px; alt="Loading" />').rakbox({
		autoOpen: true,
		width: 440,
		beforeClose: function () { prAnimateCart(myCartItemCount); }
	});
...
function RemoveItemFromCart(removeSku) {
	...
	if (location.host.indexOf("beta.rakuten.com") > -1)
		...
		removeItemLink = "http://test.cart.rakuten.com/CO/basket/cartservice.aspx?";
...
$(function () {
	...
	//buyname cookie is encrypted and can't be seen on webs so we can't check it here anymore
	//if (buyName != 'null') {
	if (superptsCookie != "null") {
		...
		if (superptsVal != "0") {
			...
			$('#headerSuperPoint').html("<div class=\"superpts-holder\"><span class=\"superpts\"><a href=\"http://www.rakuten.com/ct/Rakutensuperpoint/default.aspx\" onclick=\"s.tl(this,'o','header_Points_GetPoints');\" class=\"alert\">Get Super Points</a></span></div>");
	}
	else {
