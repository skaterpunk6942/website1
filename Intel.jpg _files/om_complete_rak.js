...
if (typeof BUY.Tracking.Cookies["SessionSource"] !== "undefined") {
    s.eVar51 = s.getValOnce(BUY.Tracking.Cookies["SessionSource"].getKeyValue("adid"), 's_evar51', 30);
    s.prop51 = s.eVar51;
...
var prodPage = (window.location.pathname.toLowerCase().indexOf("/prod/") != -1 || window.location.pathname.toLowerCase().indexOf("product.asp") != -1 || window.location.pathname.toLowerCase().indexOf("/product.aspx") != -1);
...
//bloomreach cookie settings
try{
	if (BUY.Tracking.Cookies["ClickFromIsPaid"] && BUY.Tracking.Cookies["ClickFromIsPaid"].value == "1") {
		document.cookie = "BRPaid=1; path=/; domain=.rakuten.com;";
		BUY.Tracking.Cookies["BRPaid"] = { value: "1" };
	}
	else {
		var searchEnginesList = ["google.", "bing.", "yahoo.", "ask.", "search.aol.", "yandex.", "naver.", "dogpile."];
		if (document.referrer) {
			var referrerHostName = document.referrer.split('/')[2];
			for (var i=0,j=searchEnginesList.length; i<j; i++) {
				if (referrerHostName.indexOf(searchEnginesList[i]) != -1) {
					document.cookie = "BRPaid=; path=/; domain=.rakuten.com; expires=Thu, 01 Jan 1970 00:00:01 GMT";
					break;
				}
			}
		}
	}
	document.cookie = "ClickFromIsPaid=; path=/; domain=.rakuten.com; expires=Thu, 01 Jan 1970 00:00:01 GMT"
	
	if (BUY.Tracking.Cookies["BRPaid"] && BUY.Tracking.Cookies["BRPaid"].value == "1")
		s.eVar45 = "1";
	else
		s.eVar45 = "0";
}
...
if (prodPage) {
    //s.events = 'prodView';
    if (dtmc_product_event && dtmc_product_event != "") {
        s.events = dtmc_product_event;
    } else {
        s.events = 'prodView'
    }
