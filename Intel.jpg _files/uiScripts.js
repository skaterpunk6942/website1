...
function PlayVideoReviewElements(element, flvName, swfName, height, width, startMovie, volume, imgURL, prodImgUrl, sku, channel)
{
	...
	{  // if we've detected an acceptable version
		if (startMovie == 1)
		{
			...
			if (document.getElementById(element) != null)
			{
				...
				{
					document.getElementById(element).innerHTML = '<embed name="' + swfName + '" src="http://images.rakuten.com/buy_assets/flashplayers/' + strSWFName + '.swf" loop="false" menu="false" wmode="transparent" quality="high"'
	...
	{  // flash is too old or we can't detect the plugin
		...
		{
			document.getElementById(element).innerHTML = '<a href="http://www.macromedia.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash"><img src="http://images.rakuten.com/buy_assets/buytv/flash7needed_product.jpg" alt="Requires Flash 7 or Higher" border="0" /></a>';
		}
	}
}
...
function showTabContent(tab, tabId, element, id)
{
	...
	{
		document.getElementById(tab + '_' + tabId).style.backgroundImage = 'url(http://images.rakuten.com/buy_assets/v6/menu/prod_selected_back.jpg)';
...
function changeTabBack(element)
{
	for (var x = 1; x < 15; x++)
	{
		if (document.getElementById(element + '_' + x) != null)
		{
			document.getElementById(element + '_' + x).style.backgroundImage = 'url(http://images.rakuten.com/buy_assets/v6/menu/prod_unselected_back.jpg)';
...
function chgMoreImg(thisSku, imgNum, bLrgImg, imgCount)
{
	...
	if (ssImageElement)
	{
		...
		if (imgNum > 1)
			ssImageElement.src = 'http://images.rakuten.com/db_assets/screen_shot_images/' + skuFolder + '/' + thisSku + '_' + imgNumPre + imgNum + '.jpg';
		else
			ssImageElement.src = 'http://images.rakuten.com/db_assets/large_images/' + skuFolder + '/' + thisSku + '.jpg';
	}
...
function changeAccessoryProds(elementPrefix, currentID, iCount)
{
	...
	{
		elementProdNext.innerHTML = '<a href=\"javascript:void(0);\" onclick=\"changeAccessoryProds(\'' + elementPrefix + '\',' + (currentID + 4) + ',' + iCount + ');\"><img src=\"http://images.rakuten.com/buy_assets/v6/header/2009/elements/topsellers/rightON.png\" align=\"top\" border=\"0\" alt=\"topsellers back button\" /></a>';
	} else
	{
		elementProdNext.innerHTML = '<img src=\"http://images.rakuten.com/buy_assets/v6/header/2009/elements/topsellers/rightOFF.png\" align=\"top\" border=\"0\" alt=\"topsellers back button\" />';
	}
	if (elementProdBack && currentID > 4)
	{
		elementProdBack.innerHTML = '<a href=\"javascript:void(0);\" onclick=\"changeAccessoryProds(\'' + elementPrefix + '\',' + (currentID - 4) + ',' + iCount + ');\"><img src=\"http://images.rakuten.com/buy_assets/v6/header/2009/elements/topsellers/leftON.png\" align=\"top\" border=\"0\" alt=\"topsellers next button\" /></a>';
	} else
	{
		elementProdBack.innerHTML = '<img src=\"http://images.rakuten.com/buy_assets/v6/header/2009/elements/topsellers/leftOFF.png\" align=\"top\" border=\"0\" alt=\"topsellers next button\" />';
	}
...
function showSellerListings(p_sku, p_sellerId, p_listingId, p_blnChangeVisibility)
{
	if (p_sku != "" && p_sku != null && $('#divSellerListings').html() == "")
	{
		var prodViewDate = new Date();
		$.get('/PR/SellerListingsAjax.aspx?sku=' + p_sku + '&sellerid=' + p_sellerId + '&guid=' + prodViewDate.getTime(), function (data)
		{
			if (p_listingId != null)
			{
				$('#divSellerListings').html(data).show();
				$('#spanScrollTo' + p_listingId).ScrollTo(800);
			}
			else
				$('#divSellerListings').html(data).show().ScrollTo(800);
			$('#divAllBuyingChoices img').attr("src", "http://images.rakuten.com/buy_assets/v10/product/buying_less_options_button.png");
		});
	}
	else if (!$('#divSellerListings').is(':visible'))
	{
		if (p_listingId != null)
		{
			$('#divSellerListings').show();
			$('#spanScrollTo' + p_listingId).ScrollTo(800);
		}
		else
			$('#divSellerListings').show().ScrollTo(800);
		$('#divAllBuyingChoices img').attr("src", "http://images.rakuten.com/buy_assets/v10/product/buying_less_options_button.png");
	}
	else if (p_listingId != null)
			$('#spanScrollTo' + p_listingId).ScrollTo(800);
	else
	{
		$('#divSellerListings').hide();
		$('#divAllBuyingChoices img').attr("src", "http://images.rakuten.com/buy_assets/v10/product/buying_options_button.png");
	}
}
